...
class Unet_Evid(nn.Module):
    ...
    def __init__(
        ...
        self.output_gamma = nn.Sequential(
        nn.Conv2d(ch, in_chans, kernel_size=3, padding=1, stride=1)
        )
        self.output_v = nn.Sequential(
        nn.Conv2d(ch, 1, kernel_size=3, padding=1, stride=1),
        nn.Softplus()
        )
    ...
    def forward(self, image: torch.Tensor) -> torch.Tensor:
        ...
        gamma = self.output_gamma(output)
        v = self.output_v(output)
        alpha = self.output_alpha(output) + 1
        gamma = utils.chan_dim_to_complex(gamma)        
