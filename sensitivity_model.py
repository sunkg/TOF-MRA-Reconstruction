...
class SMEB(nn.Module):
    ...
    def forward(
        ...
        num_low_frequencies: tuple,
    ) -> torch.Tensor:
        # get ACS signals only (i.e. preserve low freq only)
        ACS_mask = torch.zeros(masked_kspace.shape[-2:])
        ACS_mask[:num_low_frequencies[0], :num_low_frequencies[1]] = 1

        ACS_mask = torch.roll(ACS_mask, ACS_mask.shape[0]//2-num_low_frequencies[0]//2, dims = 0)
        ACS_mask = torch.roll(ACS_mask, ACS_mask.shape[1]//2-num_low_frequencies[1]//2, dims = 1) 
        ACS_mask = ACS_mask[None, None, :].to(masked_kspace)

        ACS_kspace = ACS_mask * masked_kspace
        # convert to image space
        ACS_images = utils.ifftn(ACS_kspace)

        # estimate sensitivities independently
        N, C, H, W = ACS_images.shape
        batched_channels = ACS_images.reshape(N*C, 1, H, W)
        
        sensitivity = self.norm_net(batched_channels)
...
class NormNet(nn.Module):
    ...
    def pad(
        ...
        _, _, h, w = x.shape
        w_mult = ((w - 1) | 2**self.num_steps-1) + 1
        h_mult = ((h - 1) | 2**self.num_steps-1) + 1
...
class Unet(nn.Module):
    ...
    def forward(self, image: torch.Tensor) -> torch.Tensor:
        ...
        for up_conv, conv in zip(self.upsampling_conv, self.up_conv):
            ...
            output = F.interpolate(output, scale_factor=2, mode='bicubic')
